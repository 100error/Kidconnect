...
export default function ProfileScreen() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      const updatedProfile = await profileService.updateProfile(username.trim(), selectedAvatar);
  ...
  return (
    <ImageBackground source={BACKGROUND_IMAGE} style={styles.bgContainer}>
      <View style={styles.overlay}>
        <SafeAreaView style={styles.container}>
          <View style={styles.header}>
            <TouchableOpacity onPress={isUpdate ? () => setIsEditing(false) : handleBack} style={styles.backButton}>
              <Ionicons name="arrow-back" size={28} color="#333" />
            </TouchableOpacity>
